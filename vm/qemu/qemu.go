...
var archConfigs = map[string]*archConfig{
	"linux/amd64": {
		...
		// "-cpu host" will panic booting the old kernel
		QemuArgs: "-enable-kvm",
...
func ctor(env *vmimpl.Env) (vmimpl.Pool, error) {
	...
	if cfg.Count < 0 || cfg.Count > 128 {
		return nil, fmt.Errorf("invalid config param count: %v, want [0, 128]", cfg.Count)
	}
	// communication check
	if cfg.CommType != "virtio" {
		return nil, fmt.Errorf("invalid communication type: %v", cfg.CommType)
	}
	// qcow2 support snapshot
	// TODO: how to check image type is qcow2?
...
func (pool *Pool) Create(workdir string, index int) (vmimpl.Instance, error) {
	...
	for i := 0; ; i++ {
		inst, err := pool.ctor(workdir, sshkey, sshuser, index, loadvm)
		if err == nil {
			return inst, nil
		}
...
func (pool *Pool) ctor(workdir, sshkey, sshuser string, index int, loadvm bool) (vmimpl.Instance, error) {
	var err error

	// Make a copy of image.
	// QEMU doesn't support combining -snapshot and loadvm, so each VM will have own copy of image.
	// Another solution: QEMU support combining -snapshot with external snapshot, but not sure about its performance.
	imageCopy := path.Join(workdir, "image")
	err = osutil.CopyFile(pool.env.Image, imageCopy)
	if err != nil {
		return nil, fmt.Errorf("cannot copy image from %s to %s: %s", pool.env.Image, imageCopy, err)
	}
	inst := &instance{
		index:      index,
		cfg:        pool.cfg,
	...
	inst.commType = pool.cfg.CommType
	// init communication
	if pool.cfg.CommType == "virtio" {
		for i := 0; i < pool.env.CommNum; i++ {
			p, err := initVirtioHost(i, workdir)
			if err != nil {
				return nil, fmt.Errorf("cannot create virtio host fifo: %v", err)
			}
			inst.virtioSerial = append(inst.virtioSerial, p)
		}
	}
...
func (inst *instance) Image() string {
	return inst.image
}

func (inst *instance) CommType() string {
	return inst.commType
}

func (inst *instance) Comm(index int) (h comm.HostComm) {
	if inst.commType == "virtio" {
		h = inst.virtioSerial[index]
	}
	return
}

func (inst *instance) RawRun(command string) (*exec.Cmd, io.WriteCloser, io.ReadCloser, io.ReadCloser, error) {
	r_stdin, stdin, err := osutil.LongPipe()
	if err != nil {
		return nil, nil, nil, nil, err
	}
	stdout, w_stdout, err := osutil.LongPipe()
	if err != nil {
		return nil, nil, nil, nil, err
	}
	stderr, w_stderr, err := osutil.LongPipe()
	if err != nil {
		return nil, nil, nil, nil, err
	}

	// TODO: is it safe to close these pipes?
	// defer func() {
	// 	w_stdout.Close()
	// 	w_stderr.Close()
	// 	r_stdin.Close()
...
func (inst *instance) Close() {
	...
	if inst.commType == "virtio" {
		for _, p := range inst.virtioSerial {
			p.Close()
		}
	}
}
...
func (inst *instance) boot() error {
	...
	// The old network configurations might trigger 'Failed to raise network interfaces'
	// in some kernels.
	args = append(args,
		"-net", "nic,model=e1000",
