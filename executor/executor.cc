...
const int kMemtracePktSize = 320;
const int kCoverSize = 256 << 10;
const int kMemtraceSize = 0x40000 * kMemtracePktSize; // put it here as common_linux.h will refer to it to set rlimit
const int kSctraceSize = 8 << 20; // put it here as common_linux.h will refer to it to set rlimit
...
static void write_size();
void write_memtrace(uint32* num, int n, char *data);
void write_sctrace(uint32* num, int n, char *data);
static uint32 hash(uint32 a);
static bool dedup(uint32 sig);
#endif

uint64 start_time_ms = 0;
uint64 start_execute_one_time_ms = 0;
...
static bool flag_memtrace;
static bool flag_sctrace;
...
static bool flag_interleave;
static bool flag_victim_interleave;
static uint8 flag_interleave_pause;
...
static uint64 interleave_program_timeout_ms;
...
int main(int argc, char** argv)
{
	...
	receive_execute();
#endif
	debug("executor start, ruid = %d, rgid = %d, euid = %d, egid = %d\n", getuid(), getgid(), geteuid(), getegid());
	do {
		void* preferred = (void*)(0x1c1dc20000ull + (1 << 20) * (getpid() % 128) + (256 << 20));	
		init_sclog(32<<10);
		for (int i = 0; i < kMaxThreads; i++) {
			threads[i].sctrace.data = (char *)mmap(preferred, kSctraceSize,
					PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED | MAP_ANON, 0, 0);
			if (threads[i].sctrace.data != preferred)
				fail("mmap of sctrace buf failed");
			threads[i].sctrace.f = fmemopen((void*)threads[i].sctrace.data, kSctraceSize, "w");
			if (threads[i].sctrace.f == NULL)
				fail("get file pointer from sctrace buf failed");
			set_log_file(i, threads[i].sctrace.f);
			preferred = (void*)((uint64)(preferred)+2*kSctraceSize);
		}
	} while(0);
	do {
		void* preferred = (void*)(0x1b2bc20000ull + (1 << 20) * (getpid() % 128) + (256 << 20));	
		for (int i = 0; i < kMaxThreads; i++) {
			threads[i].memtrace.num = 0;
			threads[i].memtrace.data = (char *)mmap(preferred, kMemtraceSize,
					PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED | MAP_ANON, 0, 0);
			if (threads[i].memtrace.data != preferred)
				fail("mmap of memtrace buf failed");
			preferred = (void*)((uint64)(preferred)+2*kMemtraceSize);
		}
	} while(0);
	if (flag_coverage) {
		for (int i = 0; i < kMaxThreads; i++) {
...
void receive_execute()
{
	...
	interleave_program_timeout_ms = 2 * program_timeout_ms;
	...
	flag_interleave = req.exec_flags & (1 << 6);
	flag_interleave_pause = (req.exec_flags >> 7) & 0xff;
	flag_memtrace = req.exec_flags & (1 << 15);
	flag_sctrace = req.exec_flags & (1 << 16);
	if (!flag_threaded)
		flag_collide = false;
...
void execute_one()
{
	...
	write_output(8); // Size of output. 
#endif
	uint64 start = current_time_ms();
	start_execute_one_time_ms = start;
...
retry:
	...
	for (;;) {
		...
		memset(&call_props, 0, sizeof(call_props));
		if (IS_VICTIM(procid) && flag_interleave && (call_index-1) == flag_interleave_pause) {
			debug("call = %d, notify...\n", call_index-1);
			debug("notify...\n");
			if (exec_notify()) {
				debug("cannot notify attacker");
			}
			debug("pause...\n");
			if (exec_wait_nosig(program_timeout_ms)) {
				debug("cannot wait attacker");
			}
			debug("continue!\n");
		}
	}
...
void write_call_output(thread_t* th, bool finished)
{
	...
	uint32* memtrace_count_pos = write_output(0); // filled in later since need to check output buffer size and flag
	uint32* sctrace_count_pos = write_output(0); // filled in later since need to check output buffer size and flag
	write_output(uint32(th->end_time_ms-start_execute_one_time_ms));
	...
	if (flag_memtrace) {
		write_memtrace(memtrace_count_pos, th->memtrace.num, th->memtrace.data);
	}
	if (flag_sctrace) {
		write_sctrace(sctrace_count_pos, ftell(th->sctrace.f), th->sctrace.data);
	}
...
void execute_call(thread_t* th)
{
	...
	if (flag_sctrace) {
		fseek(th->sctrace.f, 0, SEEK_SET);
		if (!call->call) {
			log_syscall_with_index(th->id, th->call_index, call->sys_nr, 6, th->args, 0, 0, true);
		}
	}
	if (flag_coverage)
		cover_reset(&th->cov);
	...
	th->res = res;
	if (flag_memtrace) {
		th->memtrace.num = syscall(667, 1, th->memtrace.data);
		if (th->memtrace.num < 0) {
		// Could happen when:
		// 1. Buffer is too small.
		// 2. Just execute `fork`/`clone` and this is the child process.
		// Ignore these errors.
			th->memtrace.num = 0;
		}
	}
	...
	if ((th->res == -1 && th->reserrno == 0) || call->attrs.ignore_return)
		th->reserrno = EINVAL;
	// errno is set to EFAULT before the syscall, but we don't want to count that when syscall succeeded.
	// this incorrect errno when syscall succeeded could let syscall decoding fail.
	// write_call_output also does this...
	if ((th->res != -1) && th->reserrno) {
		th->reserrno = 0;
	}
	...
	if (flag_sctrace) {
		if (!call->call) {
			log_syscall_with_index(th->id, th->call_index, call->sys_nr, 6, th->args, th->res, th->reserrno, false);
		} else {
			// TODO: is return value & error number enough for pseudo-syscalls?
			log_syscall_printf(th->id, "%d: %s() = %d {%d}\n", th->call_index, call->name, th->res, th->reserrno);
		}
	}

	th->end_time_ms = current_time_ms();
...
void write_size()
{
	__atomic_store_n(output_data+1, (uint32)((uint64)output_pos-(uint64)output_data), __ATOMIC_RELEASE);
}


void write_memtrace(uint32* num, int n, char *data) {
	uint32 len = ((n + 15) / 16) * kMemtracePktSize;
	if (((char*)(output_pos) + len) >= (char*)output_data + kMaxOutput) {
		return;
	}
	memcpy(output_pos, data, len);
	// ensure that the packet size is multiples of `sizeof(uint32)`
	output_pos = (uint32*)((char*)(output_pos)+len);
	*num = n;
}

void write_sctrace(uint32* num, int n, char *data) {
	if (((char*)(output_pos) + n) >= (char*)output_data + kMaxOutput) {
		return;
	}
	memcpy(output_pos, data, n);
