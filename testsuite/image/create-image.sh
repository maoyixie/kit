...
DIR=chroot
PREINSTALL_PKGS=vim,openssh-server,curl,tar,gcc,libc6-dev,time,strace,sudo,less,psmisc,selinux-utils,policycoreutils,checkpolicy,selinux-policy-default,firmware-atheros,debian-ports-archive-keyring
...
# Create udev rule to create virtio serial symlink
echo -e 'ATTR{name}=="vim2m", SYMLINK+="vim2m"\nKERNEL=="vport*", ATTR{name}=="?*",     SYMLINK+="virtio-ports/$attr{name}"' | sudo tee -a $DIR/etc/udev/rules.d/50-udev-default.rules
